...
- 开发步骤是怎样的？
  ...
  - 第六步：编写一个Java程序，这个小Java程序也不能随意开发，这个小java程序必须实现Servlet接口。
    ...
    - ```java
      ...
      import javax.servlet.*;//注意我的版本不是tomcat10而是9，所以任然还是导入javax包而不是jakarta
      import java.io.*;
  ...
  - 浏览器上编写的路径太复杂，可以使用超链接(``` <a href="/crm/helloservlet">Hello Servlet!</a>``` )。（**非常重要：html页面只能放到WEB-INF目录外面。**）
  ...
  - 浏览器发送请求，到最终服务器调用Servlet中的方法，是怎样的一个过程？（以下这个过程描述的很粗糙。其中还有很多步骤我省略了。）
    ...
    - Tomcat服务器找到crm项目
    - Tomcat服务器在web.xml文件中查找/crm/helloservlet   对应的Servlet是：com.bjpowernode.servlet.HelloServlet
...
```java
public void service(ServletRequest request, ServletResponse response){
    response.setContentType("text/html");// 要在获取输出流之前就要设置好内容Type
...
- 思考：我们自己new的Servlet对象受WEB容器的管理吗？
  ...
  - 我们自己new的Servlet对象是`不受`WEB容器管理的。
  - WEB容器创建的Servlet对象，这些Servlet对象都会被放到一个`集合`当中（HashMap），只有放到这个HashMap集合中的Servlet才能够被WEB容器管理，自己new的Servlet对象不会被WEB容器管理。（自己new的Servlet对象不在容器当中）
    ...
    - ```java
      public class Tomcat {
          public static void main(String[] args){
              ...
              ServletConfig servletConfig = new org.apache.catalina.core.StandardWrapperFacade();//接口的引用指向实现类的对象
...
- init方法的ServletConfig servletConfig参数, 小猫咪创建好ServletConfig实现对象后调用init()
  - 这个ServletConfig对象目前在init方法的参数上,属于局部变量, 那么ServletConfig对象肯定以后要在service方法中使用, `怎么才能保证ServletConfig对象在service()中使用?`
...
- 思考: 我又没有可能需要我在LoginServlet中重写init方法
  - 如果子类重写父类init方法,那么势必走子类init方法, 如果走子类的init方法, 父类init方法不走,也就意味着小猫咪传不了ServletConfig对象, 子类service方法的ServlertConfig就没有了
...
- 研究：各个Servlet对象是否共享同一个ServletConfig对象？
  - 经过测试， 每个servlet对象是单独与一个ServletConfig一 一对应
...
- 研究：ServletContext是否是共享的
  - 经过测试， AServlert和BServlet都打印出同一个对象，所以是共享的
...
- ServletContext接口中有哪些常用的方法？
  ...
  - ```java
    // 获取应用的根路径（非常重要），因为在java源代码当中有一些地方可能会需要应用的根路径，这个方法可以动态获取应用的根路径(/表示从web下开始找)
...
- 转发和重定向有什么区别？
  ...
  - 代码上有什么区别？
    ...
    - 重定向
      ...
      - ![image-2023025205124568](./课堂笔记imgs/image-20230925205124568.png)
...
- 当用户关闭浏览器的时候，session对象不会立马销毁，而是运行着session超时机制，超过一定时间没有请求才会销毁session
...
- ![image-20231005141150094](./课堂笔记imgs/image-20231005141150094.png)

- ![image-20231005141925233](./课堂笔记imgs/image-20231005141925233.png)
...
- 不让jsp生成session内置对象(不能写！会导致yong'bu)
...
- 关于cookie的path，cookie关联的路径：`(涉及重定向必须要注意cookie关联路径，不然不会携带你写的cookie)`
...
- 对JSP进行错误调试的时候，还是要直接打开JSP文件对应的java文件，检查java代码。`放在CATALINA_BASE路径`
...
- EL表达式的使用：
  ...
  - 通过EL表达式获取应用的根：
    ...
    - EL表达式没有HttpServletRequest对象，requestScope代表请求范围不代表HttpServletRequest对象。
    - EL表达式有一个pageContext对象，是和jsp九大内置对象中的pageContext是同一个对象
    - pageContext对象有很多get方法比如`pageContext.getRequest()`可以获取ServletRequest对象
...
- 老杜的思路是先用传统的方式开发一个银行转账功能，开发好后再使用mvc设计模式来开发这个功能

- 没有使用mvc存在的问题![image-20231110134159529](./课堂笔记imgs/image-20231110134159529.png)

- MVC理论基础
  - 横向看是mvc设计模式![image-20231110143102347](./课堂笔记imgs/image-20231110143102347.png)

    - dao

      > ![image-20231110154224448](./课堂笔记imgs/image-20231110154224448.png)
      >
      > ![image-20231111170655569](./课堂笔记imgs/image-20231111170655569.png)
      >
      > 对bean实现crud数据库操作,专门数据库crud

    - bean

      > 对某一个表的映射类, 每一条数据对应一个这个类的对象, 专门数据封装

    - service

      > ![image-20231111170905077](./课堂笔记imgs/image-20231111170905077.png)
      >
      > 使用dao实现业务,专门实现业务

  - 竖着看是叫三层架构

  ![image-20231112101901533](./课堂笔记imgs/image-20231112101901533.png)

  - mvc设计模式与三层架构的关系模式图![image-20231112102016791](./课堂笔记imgs/image-20231112102016791.png)
  - ![image-20231112102558476](./课堂笔记imgs/image-20231112102558476.png)
  - ![image-20231112102635011](./课堂笔记imgs/image-20231112102635011.png)
  - ![image-20231112120237221](./课堂笔记imgs/image-20231112120237221.png)

- 之前老韩封装了BacicDAO

- 使用mvc思想改进mvc01项目

  - 看idea中mvc02的项目代码(mvc01- 使用dao bean)

  - 在mvc中使用事务

    1. 如果要使用事务，就必须在service里写事务，写jdbc代码（暂时）

    2. 写完jdbc代码存在service和dao中connnection对象不一致的问题，我们可以通过传参解决，但是代码丑陋，我不想通过传参的方式

    3. 那我们发现service的线程和调用dao的线程是同一个，如果我们把conn与thread对象绑定（map集合）

    ![image-20231112122858285](./课堂笔记imgs/image-20231112122858285.png)

    4. 我们手写了一个MyThreadLocal，来绑定线程和Connection对象

    5. 事务的问题流//TODO

       >现在没有事务，怎么解决？
       >
       >​	在service层中编写jdbc代码控制事务
       >
       >为什么不是在dao层？
       >
       >​	因为dao不涉及业务， 而业务层需要一些业务逻辑执行完成后再提交数据，如果写在dao写	死了执行完就提交。不方便业务/事务开展
       >
       >如果在service里编写jdbc获取的connection对象和在dao中的connection对象并不一样,会导致什么?
       >
       >​	会导致无法控制到dao对应connection的事务
       >
       >怎么解决connection对象不一致的问题?
       >
       >​	第一种方案: 通过给每个dao方法传参connection
       >
       >​		由于第一种每次调用dao crud方法都需要传入connection对象很丑,而且麻烦
       >
       >​		所有第二种方案不仅可以解决connection不一致的问题,而且代码更加美观
       >
       >​	第二种方案: 使用ThreadLocal
